...
// テスト対象/decline_friend?id=1&friend_id=2 response: 200 "Friend request declined" or 500 "Failed to decline friend request"
func TestDeclineFriendIntegration(t *testing.T) {
	// 初期設定
	e := echo.New()
	conf := configs.Get()
	...
	aliceID, bobID, cleanupFunc, err := setupTestDataForDeclineFriend(db)
	if err != nil {
	...
	client := &http.Client{}
	req, err := http.NewRequest("POST", fmt.Sprintf("%s/decline_friend?id=%d&friend_id=%d", ts.URL, bobID, aliceID), nil)
	if err != nil {
	...
	testhelpers.AssertEqual(t, "Friend request declined", string(bodyBytes))
}

func setupTestDataForDeclineFriend(db *sql.DB) (int64, int64, func(), error) {
